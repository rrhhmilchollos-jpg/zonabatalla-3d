...
interface GameStore extends GameSessionState {
  ...
  setCollisionObjects: (updater: (prev: THREE.Object3D[]) => THREE.Object3D[]) => void; // A setter for collision objects

  // Touch controls (now handled by useTouchControls hook directly)
}
...
const useGameStore = create<GameStore>((set, get) => ({
  ...
  setCollisionObjects: (updater) => set((state) => ({
    collisionObjects: updater(state.collisionObjects)
  })),
}));
